...
export function TikTokReviewCreationDialog({ open, onOpenChange }: TikTokReviewCreationDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (urls.length === 0) {
      toast({
        title: "错误",
        description: "请至少添加一个TikTok商品URL",
        variant: "destructive",
      })
      return
    }
    ...
    console.log('无效的URLs:', invalidUrls)
    
    if (invalidUrls.length > 0) {
      toast({
        title: "URL格式错误",
        description: `以下URL格式不正确，请使用TikTok Shop商品链接：\n${invalidUrls.join('\n')}`,
        variant: "destructive",
      })
      return
    }
    ...
    try {
      console.log('发送请求到 /api/tiktok-reviews/tasks')
      const requestBody = {
        urls,
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-2xl font-bold">创建TK评论爬虫任务</DialogTitle>
              <DialogDescription className="text-base mt-1">
                输入TikTok Shop商品URL开始评论数据抓取，支持批量处理多个商品
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-2 text-sm text-muted-foreground">
              ...
              <div>
                <p>• 每行输入一个TikTok Shop商品URL</p>
                <p>• 支持同时处理多个商品页面</p>
                <p>• URL格式：https://shop.tiktok.com/view/product/商品ID</p>
              </div>
            </div>
          </div>
